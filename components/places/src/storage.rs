...
pub mod history_sync {
    ...
    pub fn apply_synced_visits(db: &Connection,
                               new_guid: &SyncGuid,
    ...
    pub fn fetch_outgoing(db: &Connection,
        ...
        let mut stmt = db.conn().prepare(&format!("
            ...
            LIMIT :max_places", (SyncStatus::Normal as u8)))?;
        ...
        let ts_rows = tombstones_stmt.query_map_named(
                            &[(":max_places", &(max_places as u32))],
                            |row| SyncGuid(row.get::<_, String>("guid")))?;
        for r in ts_rows {
            let guid = r?;
        ...
        for t in rows {
            ...
            if visits.len() == 0 {
                info!("Page {:?} is flagged to be uploaded, but has no visits - skipping", &page.guid);
                continue;
            }
            trace!("outgoing record {:?}", &page.guid);
            ids_to_update.push(page.row_id);
            db.execute_named_cached(insert_meta_sql,
                                    &[(":row_id", &page.row_id),
                                      (":change_delta", &page.sync_change_counter)])?;

            result.insert(page.guid.clone(), OutgoingInfo::Record(HistoryRecord {
                id: page.guid,
                title: page.title,
        ...
        sql_support::each_chunk(&ids_to_update, |chunk, _| -> Result<()> {
            db.conn().execute(&format!("UPDATE moz_places SET sync_status={status}
                                 WHERE id IN ({vars})",
    ...
    pub fn finish_outgoing(db: &Connection) -> Result<()> {
        ...
        db.execute_all(&[&format!("UPDATE moz_places
...
mod tests {
    ...
    fn get_custom_observed_page<F>(conn: &mut PlacesDb, url: &str, custom: F) -> Result<PageInfo>
        where F: Fn(VisitObservation) -> VisitObservation {
        let u = Url::parse(url)?;
        let obs = VisitObservation::new(u.clone()).with_visit_type(VisitTransition::Link);
        apply_observation(conn, custom(obs))?;
        Ok(fetch_page_info(conn.conn(), &u)?.expect("should have the page").page)
    }
    ...
    fn test_visit_counts() -> Result<()> {
        ...
        let rid1 = apply_observation(&mut conn, VisitObservation::new(url.clone())
                    .with_visit_type(VisitTransition::Link)
                    .with_at(Some(late_time.into())))?.expect("should get a rowid");

        let rid2 = apply_observation(&mut conn, VisitObservation::new(url.clone())
                    .with_visit_type(VisitTransition::Link)
                    .with_at(Some(early_time.into())))?.expect("should get a rowid");
        ...
        let rid3 = apply_observation(&mut conn, VisitObservation::new(url.clone())
                    ...
                    .with_is_remote(true))?.expect("should get a rowid");
    ...
    fn test_change_counter() -> Result<()> {
        ...
        apply_observation(&mut conn, VisitObservation::new(pi.url.clone())
    ...
    fn test_status_columns() -> Result<()> {
        ...
        conn.execute_named_cached("UPDATE moz_places
                                   SET frecency = 100
                                   WHERE id = :id",
                                  &[(":id", &pi.row_id)])?;
        ...
        pi2 = fetch_page_info(&conn, &pi2.url)?.expect("page should exist").page;
        ...
        pi3 = fetch_page_info(&conn, &pi3.url)?.expect("page should exist").page;
    ...
    fn test_sync_reset() -> Result<()> {
        ...
        let _ = env_logger::try_init();
        let mut pi = get_observed_page(&mut conn, "http://example.com")?;
        conn.execute_cached(&format!("UPDATE moz_places set sync_status = {}", (SyncStatus::Normal as u8)), &[])?;
        pi = fetch_page_info(&conn, &pi.url)?.expect("page should exist").page;
        assert_eq!(pi.sync_change_counter, 1);
    ...
    fn test_apply_synced_deletion_new() -> Result<()> {
        ...
        assert!(fetch_page_info(&conn, &pi.url)?.is_none(), "should have been deleted");
